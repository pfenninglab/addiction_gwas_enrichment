...
def main(args):
    ...
    lab = f'{args.label}_OCP_NB{args.batch_size}_NE{args.epochs}_BR{args.base_lr}_MR{args.max_lr}_BM{args.base_m}_MM{args.max_m}'
    model_name = f"cnn/singleTask/{args.label}/{lab}.h5"
    # call main functions
    if args.mode == 'train':
        ...
        model_test_performance = f'predictions/cnn/{args.label}/{lab}_testPerformance.feather'
        ...
        model_predictions = f'predictions/cnn/{args.label}/{lab}_testPredictions.txt'
        # save model performances to feather object
        if not os.path.exists(f'predictions/cnn/{args.label}'):
            os.makedirs(f'predictions/cnn/{args.label}')
