...
for model_name in sorted(glob.glob('cnn/singleTask/*/*.h5')):
	featherFile = model_name.replace('cnn/singleTask','predictions/cnn').replace('.h5','_testPerformance.feather')
...
df.reset_index().to_feather('rdas/SingleTask_IDR_peakPrediction_CNN_performance.feather')
df.reset_index().to_csv('tables/SingleTask_IDR_peakPrediction_CNN_performance.tsv', sep = '\t')
