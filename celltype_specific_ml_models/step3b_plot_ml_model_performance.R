...
####################
# read in the data
df = read.delim('tables/SingleTask_IDR_peakPrediction_CNN_v2_performance.tsv')
...
df$dropout = factor(ss(ss(basename(df$model), '_DO',2), '\\.h5'))

pdf('plots/mouse_ml_model-CNN_v2_performance.pdf', width = 11, height = 5)
...
pdf('figures/figure_mouse_ml_model-CNN_v2_performance.pdf', 
    width = 5.5, height = 4.75)
ggplot(data = df_long, aes(x = celltype, y = value)) + 
  geom_boxplot(aes(color = celltype)) + 
  geom_jitter(width = .25, pch = 21, aes(color = celltype)) + 
